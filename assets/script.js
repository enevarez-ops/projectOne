$(document).ready(function () {
  ...
  function getRecipeRepos() {
    ...
    fetch(requestUrl)
      ...
      .then(function (data) {
        console.log(data);
        var randomEl = $("<a>").text(data.meals[0].strYoutube);
        var recipeImg = $("<img>").attr("src", data.meals[0].strMealThumb);
        var recipeSrc = $("<a>")
          .text("" + data.meals[0].strMeal)
          .attr("href", data.meals[0].strSource)
          .attr("target", "_blank");
        var recipeCat = $("<h3>")
          .text("Category: " + data.meals[0].strCategory)
          .attr("header", data.meals[0].strCategory);
  ...
  function getRandomMusic() {
    ...
    $.ajax(settings).done(function (response) {
      ...
      var link = $("<a>")
        .attr("href", newObj.link)
        .attr("title", "album link")
        .text(newObj.album.title);
